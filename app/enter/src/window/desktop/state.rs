...
pub struct State {
    ...
    desktop_frame_scaled: bool,// 是否已被缩放
    desktop_frame_scalable: bool,// 可缩放
    render_rx: Receiver<DesktopDecodeFrame>,
    frame_slot: Arc<Mutex<DesktopDecodeFrame>>,// 存储帧数据
    frame_size: (i32, i32),
}
...
impl State {
    ...
    pub fn update_desktop_frame(&mut self) -> (i32, i32) {
        ...
        // while let Ok(mut guard) = self.render_rx.lock() {
